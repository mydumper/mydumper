...
#ifdef WITH_SSL
static void check_pem_exists(const char *filename, const char *option)
{
  if (!filename)
...
static void check_capath(const char *path)
{
...
static void m_options(MYSQL *conn, enum mysql_option option, gboolean arg_b, const void *arg_v)
{
  if (arg_b)
  {
