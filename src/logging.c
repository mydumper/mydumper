...
static void append_json_field(GString *json, gboolean *first, const gchar *key, const gchar *value, gssize len)
{
  if (!(*first))
  {
    g_string_append_c(json, ',');
...
static void append_json_bool_field(GString *json, gboolean *first, const gchar *key, gboolean value)
{
  if (!(*first))
  {
    g_string_append_c(json, ',');
...
static void append_json_int_field(GString *json, gboolean *first, const gchar *key, gint64 value)
{
  if (!(*first))
  {
    g_string_append_c(json, ',');
...
static const GLogField *find_log_field(const GLogField *fields, gsize n_fields, const gchar *key)
{
  gsize i = 0;
    ...
    gpointer                                                   user_data)
{
  ...
  phase_name = phase_field != NULL && phase_field->value != NULL ? field_string_value(phase_field) : "runtime";
  status_name = status_field != NULL && status_field->value != NULL ? field_string_value(status_field) : default_runtime_status(log_level);
