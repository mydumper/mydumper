...
void string_pk_planner_reset_for_table(struct db_table *dbt, guint64 rows)
{
  ...
  dbt->string_planner_deadline_us =
      string_pk_planner_timeout_seconds > 0 ? dbt->string_planner_started_us + ((gint64)string_pk_planner_timeout_seconds * G_USEC_PER_SEC) : 0;
 ...
 */
static GList *probe_prefix_children(MYSQL *conn, struct db_table *dbt, const struct prefix_estimate *victim, guint current_count, const gchar *alphabet, guint *child_count, gboolean *over_budget, gboolean *aborted)
{
  GList *children = NULL;
  *child_count = 0;
  *over_budget = FALSE;
...
gboolean string_pk_plan_prefix_chunks(MYSQL *conn, struct db_table *dbt, guint64 rows)
{
  ...
  dbt->status = READY;
  g_message(
      "String PK planner selected metadata-assisted prefix chunks for %s.%s "
