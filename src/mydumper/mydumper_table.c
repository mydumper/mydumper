...
void finalize_table()
{
  ...
  if (json_fields_cache)
    g_hash_table_destroy(json_fields_cache);
  if (generated_fields_cache)
    g_hash_table_destroy(generated_fields_cache);
}
...
static gchar *get_character_set_from_collation(MYSQL *conn, gchar *collation)
{
  g_mutex_lock(character_set_hash_mutex);
...
static void get_primary_key(MYSQL *conn, struct db_table *dbt, struct configuration *conf)
{
...
static void get_primary_key_separated_by_comma(struct db_table *dbt)
{
...
static GString *get_selectable_fields(MYSQL *conn, char *database, char *table)
{
  MYSQL_ROW row;
...
static void replace_select_fields(GString *select_fields, GHashTable *column_replace_hash)
{
