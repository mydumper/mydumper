...
static gchar *read_word(gchar *buffer, gchar *val)
{
  guint i = 0;
  while (*val != '\0' && *val != ' ')
  {
...
struct function_pointer *init_function_pointer(gchar *value)
{
  ...
  else if (g_str_has_prefix(value, "apply"))
  {
    fp->is_pre = TRUE;
    parse_apply_function(fp, g_strdup(&(fp->value[6])));
  }
  else if (g_str_has_prefix(value, "constant"))
  {
    fp->is_pre = TRUE;
    parse_constant_function(fp, g_strdup(&(fp->value[9])));
  }
  else if (g_str_has_prefix(value, "regex"))
  {
    //    fp->is_pre=TRUE;
