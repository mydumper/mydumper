...
void initialize_process(struct configuration *c)
{
  ...
  if (max_decompressors > 32)
    max_decompressors = 32;
  if (max_decompressors < 4)
    max_decompressors = 4;
...
static gboolean parse_create_table_from_file(struct db_table *dbt, gchar *filename)
{
  void    *infile;
  gboolean eof = FALSE;
...
static void get_database_table_part_name_from_filename(const gchar *filename, gchar **database, gchar **table, guint *part, guint *sub_part)
{
  ...
  if (g_strv_length(split_db_tbl) >= 2)
  {
    ...
    if (g_strv_length(split_db_tbl) > 3)
      *sub_part = g_ascii_strtoull(split_db_tbl[3], NULL, 10);
  }
  else
  {
    *database = NULL;
...
static gchar *get_database_name_from_filename(const gchar *filename)
{
...
static gchar *get_database_name_from_content(gchar *filename, GString **out_content)
{
  FILE *infile;
...
static void get_database_table_name_from_filename(const gchar *filename, const gchar *suffix, gchar **database, gchar **table)
{
...
gint cmp_restore_job(gconstpointer rj1, gconstpointer rj2)
{
  ...
  return (sub_a > sub_b) ? 1 : (sub_a < sub_b) ? -1
                                               : 0;
}
