...
echo "
  prepare_el_mysql80:
    steps:
    - run: sudo yum install -y libasan gdb time mysql-community-libs mysql-community-devel mysql-community-client
  ...
  prepare_ubuntu_percona57:
    ...
    - run: sudo apt-get install -y gdb time libperconaserverclient20 percona-server-client-5.7 libperconaserverclient20-dev

  prepare_ubuntu_percona80:
    steps:
    - run: sudo percona-release setup -y ps80
    - run: sudo apt-get install -y gdb time libperconaserverclient21 libperconaserverclient21-dev percona-server-client


  prepare_ubuntu_mariadb1006:
    steps:
    - run: sudo apt-get install -y gdb time mariadb-client libmariadbclient18 libmariadb-dev libmariadb-dev-compat || true
    - run: sudo apt-get install -y gdb time mariadb-client libmariadbclient18 libmariadb-dev libmariadb-dev-compat

  prepare_el_mariadb1006:
    steps:
    - prepare_mariadb1006
    - run: sudo yum install -y libasan gdb time MariaDB-devel
    - run: sudo yum install -y libasan gdb time MariaDB-compat || true
...
"
  ...
  prepare_el_percona57:
    ...
    - run: yum -y install libasan gdb time Percona-Server-devel-57 Percona-Server-client-57
  ...
  prepare_el_percona80:
    ...
    - run: yum -y install libasan gdb time percona-server-devel percona-server-client
